      ...
      {showForm&&(
        <div className="add-form-box">
          <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:'1rem'}}>
            <div style={{fontSize:14,fontWeight:600,color:'var(--navy)'}}>New income entry</div>
...
function Tax({ entries, taxData, setTaxData, calcDone, onSection, toast }) {
  ...
  const calculate=()=>{
    ...
    if(filingMethod==='44AD'){
      ...
      if(!totalAmt){toast('Please enter your gross receipts');return;}
      const limit=cashPct<=5?7500000:5000000
      if(totalAmt>limit){toast(`Receipts exceed 44ADA limit. Please use Normal filing.`);return;}
      const declared=parseFloat(declaredIncome)||0
      grossIncome=Math.max(totalAmt*0.5,declared||totalAmt*0.5)
    } else {
        ...
        <div className="tax-grid">
          <div>
            <div className="panel" style={{marginBottom:'1.25rem'}}>
              ...
              {filingMethod==='44ADA'&&(
                <>
                  <div className="input-row"><label>Gross receipts (₹) — must be ≤ ₹50L (or ₹75L if 95%+ digital)</label><input className="big-input" type="number" placeholder="e.g. 600000" value={manualCash} onChange={e=>setManualCash(e.target.value)}/></div>
                  <div style={{background:'var(--amber-light)',border:'1px solid #ffc107',borderRadius:8,padding:'0.75rem 1rem',fontSize:12,color:'var(--amber)',marginBottom:'1rem'}}>
                    Minimum profit you must declare: {fmt((parseFloat(manualCash)||0)*0.5)} (50% of gross receipts)
                  </div>
                  <div className="input-row"><label>Income you want to declare (₹) — leave blank to use 50%</label><input className="big-input" type="number" placeholder={`Minimum = ${fmt((parseFloat(manualCash)||0)*0.5)}`} value={declaredIncome} onChange={e=>setDeclaredIncome(e.target.value)}/></div>
                </>
              )}
              {filingMethod==='Normal'&&(
          ...
          <div className="panel">
            <h3>Live preview</h3>
            <div className="result-row"><span className="r-label">{filingMethod==='Normal'?'Net income':'Total receipts'}</span><span className="r-val">{fmt(filingMethod==='44ADA'?parseFloat(manualCash)||0:totalAmt)}</span></div>
            {filingMethod==='44AD'&&<><div className="result-row"><span className="r-label">Min 8% on cash</span><span className="r-val">{fmt(cashAmt*0.08)}</span></div><div className="result-row"><span className="r-label">Min 6% on digital</span><span className="r-val">{fmt(digitalAmt*0.06)}</span></div><div className="result-row"><span className="r-label">Combined minimum profit</span><span className="r-val green-val">{fmt(minPresumptive)}</span></div></>}
            {filingMethod==='44ADA'&&<div className="result-row"><span className="r-label">Min profit (50%)</span><span className="r-val green-val">{fmt((parseFloat(manualCash)||0)*0.5)}</span></div>}
          ...
          <div className="panel" style={{marginBottom:'1.25rem'}}>
            ...
            {regimeChoice==='old'&&(
              <div style={{background:'var(--bg)',borderRadius:10,padding:'1rem',marginBottom:'1.25rem'}}>
                <div style={{fontSize:13,fontWeight:600,color:'var(--navy)',marginBottom:'0.75rem'}}>Enter your deductions (old regime only)</div>
                <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:8}}>
                  <div><label className="form-label">80C — LIC, PPF, ELSS, tuition fees (max ₹1.5L)</label><input className="form-input" type="number" placeholder="e.g. 150000" value={ded80C} onChange={e=>setDed80C(e.target.value)}/></div>
                  <div><label className="form-label">80D — Health insurance premium</label><input className="form-input" type="number" placeholder="e.g. 25000" value={ded80D} onChange={e=>setDed80D(e.target.value)}/></div>
                  <div><label className="form-label">HRA exemption (if applicable)</label><input className="form-input" type="number" placeholder="e.g. 0" value={dedHRA} onChange={e=>setDedHRA(e.target.value)}/></div>
                  <div><label className="form-label">80CCD(1B) — NPS additional (max ₹50K)</label><input className="form-input" type="number" placeholder="e.g. 50000" value={dedNPS} onChange={e=>setDedNPS(e.target.value)}/></div>
                  <div style={{gridColumn:'1/-1'}}><label className="form-label">Other deductions (80G donations, 80TTA interest, etc.)</label><input className="form-input" type="number" placeholder="e.g. 0" value={dedOther} onChange={e=>setDedOther(e.target.value)}/></div>
                </div>
                {totalDeductions>0&&<div style={{marginTop:8,fontSize:12,color:'var(--green)'}}>Total deductions: {fmt(totalDeductions)}</div>}
              </div>
            )}
